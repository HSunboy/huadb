...
slotid_t TablePage::InsertRecord(std::shared_ptr<Record> record, xid_t xid, cid_t cid) {
  ...
  // 返回插入的 slot id
  // LAB 1 BEGIN
  huadb::db_size_t recordSize = record->GetSize();
  *upper_ = *upper_ - recordSize;
  huadb::slotid_t nextSlotId = GetRecordCount();
  slots_[nextSlotId].offset_ = *upper_;
  slots_[nextSlotId].size_ = recordSize;
  *lower_ = *lower_ + sizeof(Slot);
  record->SetDeleted(false);
  record->SerializeTo(this->page_data_ + slots_[nextSlotId].offset_);
  this->page_->SetDirty();
  return nextSlotId;
}
...
void TablePage::DeleteRecord(slotid_t slot_id, xid_t xid) {
  ...
  record->DeserializeHeaderFrom(this->page_data_ + slots_[slot_id].offset_);
  record->SetDeleted(true);
  record->SerializeHeaderTo(this->page_data_ + slots_[slot_id].offset_);
  this->page_->SetDirty();
}
...
std::shared_ptr<Record> TablePage::GetRecord(Rid rid, const ColumnList &column_list) {
  ...
  record->SetRid(rid);
  record->DeserializeFrom(this->page_data_ + slots_[rid.slot_id_].offset_, column_list);
